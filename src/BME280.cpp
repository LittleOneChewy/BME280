...
{
   CalcMethodFloat method = m_settings.cmFloat;
   float tempOffset = m_settings.tempOffset * 5120.0;

   if (method == CalcMethodFloat_True) {
    // Double code from BME280 API (https://github.com/BoschSensortec/BME280_driver/)
    double var1;
    double var2;
    double temperature;
    ...
    t_fine = (int32_t)(var1 + var2 + tempOffset);
    temperature = (var1 + var2 + tempOffset) / 5120.0;
    ...
    return unit == TempUnit_Celsius ? temperature : temperature * 9.0 / 5.0 + 32.0;
   } else {
    ...
    t_fine = var1 + var2 + tempOffset;
    final = (t_fine * 5 + 128) >> 8;
    return unit == TempUnit_Celsius ? final/100.0 + tempOffset : final/100.0*9.0/5.0 + 32.0 + tempOffset;
   }
}
