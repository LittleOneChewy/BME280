...
## Modifications from base library
Modified only I2C module.

Added two optional parameters to the `Settings`:

`CalcMethodFloat` - defines `float` method of calculations, where is no performance penalty on 32bit controllers

`float _temp_offset = 0.0f` - temperature offset from calibrated parameters
